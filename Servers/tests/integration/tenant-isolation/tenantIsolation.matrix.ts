...
export interface EntityFixture {
  ...
  updatePayload: (owner: TenantContext) => Payload;
  /**
   * Create body. It should carry `foreignOrgId` as `organization_id`, which the
   * server must ignore. Omit only when the create op is skipped.
   */
  createPayload?: (owner: TenantContext, foreignOrgId: number) => Payload;
}
